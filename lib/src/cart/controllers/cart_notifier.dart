...
class CartNotifier with ChangeNotifier {
  ...
  Future<void> addToCart(String data, BuildContext context) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        refetchCount!();
        context.read<ColorSizesNotifier>().setSize('');
        context.read<ColorSizesNotifier>().setColor("");
        context.read<TabIndexNotifier>().setIndex(2);
        context.go('/home');
      }
    } catch (e) {
      debugPrint(e.toString());
  ...
  void selectOrDeselect(int id, CartModel cartItem) {
    if (_selectedCartItemsId.contains(id)) {
      ...
      _selectedCartItems.removeWhere((element) => element.id == id);
  ...
  void createCheckout(String data) async {
    print("Create checkout func: ${data}");
    try {
      //https://ac5a-148-251-135-91.ngrok-free.app/stripe/create-checkout-session
      Uri url = Uri.parse(
